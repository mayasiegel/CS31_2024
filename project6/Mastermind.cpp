...
{
    Score s(m, mWinningMove); // create a score object - give it move and winning move
    mBoard.endRound(m, s); // tell the board the round is over with the move and the score
    mCurrentScore = s; // update current score
    mRound++; // increment round
    return(s);
}
